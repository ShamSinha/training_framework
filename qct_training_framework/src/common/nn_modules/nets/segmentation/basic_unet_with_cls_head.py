...
class ClassHead(nn.Module):
    def __init__(self, in_channels=256, out_classes=2):
        super().__init__()
...
class BasicUNetWithClsHead(nn.Module):
    @deprecated_arg(
        name="dimensions",
        new_name="spatial_dims",
        since="0.6",
    ...
    def __init__(
        ...
        num_class_heads: Optional[int] = 0,
        class_head_num_cls: Optional[List[int]] = [],
    ):
        ...
        Args:
            ...
            bias: whether to have a bias term in convolution blocks. Defaults to True.
                According to `Performance Tuning Guide
                <https://pytorch.org/tutorials/recipes/recipes/tuning_guide.html>`_,
        ...
        self.num_class_heads = num_class_heads

        self.class_heads = torch.nn.ModuleList(
            [ClassHead(fea[4], class_head_num_cls[i]) for i in range(num_class_heads)]
        )
    ...
    def forward(self, x: torch.Tensor):
        ...
        class_logits = []
        for i in range(self.num_class_heads):
            class_logits.append(self.class_heads[i](x4))
        return seg_logits, class_logits


# BasicUnet = Basicunet = basicunet = BasicUNet
if __name__ == "__main__":
    # x = torch.randn(2, 256, 2, 2, 2).cuda()
    # net = ClassHead(256, 2).cuda()
    # y = net(x)

    # model = BasicUNetWithClsHead(
    # spatial_dims=3, in_channels=1, out_channels=2, num_class_heads=1, class_head_num_cls=[2])
    # model.eval()
    # model.cuda()
    # batch_size = 2
    # x = torch.rand((batch_size, 1, 32, 32, 32)).cuda()
    # with torch.no_grad():
    #     seg_logits, class_logits = model(x)
    #     breakpoint()
    #     print(seg_logits.shape, class_logits.shape)
