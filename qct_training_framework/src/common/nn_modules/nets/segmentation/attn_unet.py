...
class AttentionUnet(nn.Module):
    """Attention Unet based on Otkay et al. "Attention U-Net: Learning Where to Look for the
    ...
    def __init__(
        ...
        def _create_block(
            channels: Sequence[int], strides: Sequence[int], level: int = 0
        ) -> nn.Module:
            if len(channels) > 2:
                ...
                return self._get_bottom_layer(
                    channels[0], channels[1], strides[0], level=level + 1
                )
    ...
    def _get_bottom_layer(
        self, in_channels: int, out_channels: int, strides: int, level: int
    ) -> nn.Module:
        return AttentionLayer(
