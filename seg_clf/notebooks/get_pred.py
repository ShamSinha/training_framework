...
from src.common.nn_modules.nets.slicewise.multitaskold import (
    MultiTaskFusionNet,
...
for i in tqdm(range(len(test_ds))):
    try:
        ...
        for i_batch, batch in enumerate(dataloader):
            sdh_outputs = model(batch.swapaxes(0,1).unsqueeze(0).to("cuda:1"))
...
df.to_csv("fusion_v5_crop_new_test_set_v2.csv", index=False)

df_already_saved = pd.read_csv("fusion_v5_crop_new_test_set.csv")

pd.concat([df_already_saved, df]).to_csv("fusion_v5_crop_new_test_set.csv", index=False)
