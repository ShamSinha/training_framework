...
class VNet(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(out_channels, int):
            ...
            self.out_tr = nn.ModuleList(
                [
                    OutputTransition(spatial_dims, 32, out_channels[i], act, bias=bias)
    ...
    def forward(self, x):
        ...
        output = {}
        if isinstance(self.out_tr, nn.ModuleList):
            for i in range(len(self.out_tr)):
                output[self.label_keys[i]] = self.out_tr[i](x)
        else:
            output = self.out_tr(x)
