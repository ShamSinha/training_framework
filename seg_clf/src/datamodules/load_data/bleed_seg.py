            ...
            if temp_dict["scan_label"] == 1 and temp_dict["study_uid"] in study_uids_having_bleed_mask: 
                try : 
                    ...
                    if "mask" not in f2.keys():
                        ...
                        if phase == "train" :
                            if "hemi_seperator_mask" not in f3.keys() or "icv_mask" not in f3.keys():
                                continue
                        f3.close
                    else :
                        if phase == "train" :
                            if "hemi_seperator_mask" not in f2.keys() or "icv_mask" not in f3.keys():
                                continue
...
def ich_seg_data_loader_csv(
    ...
    if phase == "train" :
        df = df[df["StudyUID"].isin(study_uids_having_bleed_mask + kaggle_study_uids)]
