...
# ## load ribfracture model 
# model = torch.jit.load('/fast_data_e2e11/abhishek/qureai/packages/python/qxr/traces_ts/v4_ribfracture_cuda.ts', map_location='cuda')


# ## run ribfracture model save df 
# pred_df = run_model(model, dloader, ['ribfracture'], seg=True, save_dir="/fast_data_e2e_1/cxr/qxr_ln_data/testing_data_masks")

# output_filename = f"/fast_data_e2e_1/cxr/qxr_ln_data/testing_data_masks/ribfracture/ribfracture_scores_{process_number}.csv"
...
dset = ScanD(png_dict, 960)
dloader = DataLoader(dset, batch_size=64, num_workers=4)
...
pred_df = run_model(model,dloader, ['ribfracture'], seg=True, save_dir="/fast_data_e2e_1/cxr/qxr_ln_data/checking/")
output_filename = f"/fast_data_e2e_1/cxr/qxr_ln_data/checking/ribfracture/ribfracture_scores_2.csv"
pred_df.to_csv(output_filename, index=False)