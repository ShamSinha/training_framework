...
class Sampling_weight:
    def __init__(self, df, args) -> None:
        ...
        self.only_seg = getattr(args.seg,"only_seg", False)
    ...
    def get_cls_sampling_wts(self):
        ...
        for each_class in self.cls_classes:
            ...
            # )
            class_sampling_wts_dict[each_class] = 1 / (self.df.loc[self.df[each_class] != -100, each_class].sum() + 1e-6)
            print(
    ...
    def _annotation_ids(self):
        ...
        for each_class in self.seg_classes:
            ...
            # print(self.annotation_path, isinstance(self.annotation_path, list), type(self.annotation_path),f"{self.annotation_path}/{each_class}/*")
    ...
    def _filter_annots_entries(self):
        """filtering entries for which annotations (segmentation) are not present"""
        if self.only_seg:
            annots_train = self._annotation_ids()
            annots_train_union = set([])
            for k in self.seg_classes:
                annots_train_union = annots_train_union.union(set(annots_train[k]))
            # annots_train_union = set(
            #     [x.rsplit(".", 1)[0] for x in list(annots_train_union)]
            # )
            self.df["seg"] = self.df.index.to_series().apply(
                lambda x: 1 if x in annots_train_union else 0
            )
            print("original dataframe length", self.df.shape[0])
            for each_class in self.seg_classes:
                print(each_class.center(50, "="))
                print(self.df[each_class].value_counts())
            self.df = self.df.loc[(self.df["seg"] == 1) | (self.df["normal"] == 1) | (self.df["nota"] == 1)].copy()
            print("filtered dataframe length after only seg", self.df.shape[0])
            for each_class in self.seg_classes:
                print(each_class.center(50, "="))
                print(self.df[each_class].value_counts())
    ...
    def get_seg_sampling_wts(self):
        ...
        for each_class in [c for c in self.seg_classes if c not in ["normal", "nota"]]:
            ...
            # print(each_class, user_annot_sampling_factor)

            if not ("normal" in each_class or "nota" in each_class):
                self.df[each_class + "_annotations_sampling_wts"] = [
                    annot_sampling_factor if (x in all_annot_ids[each_class]) else 1
                    for x in self.df.index
                ]
            # print(self.df.index.to_list()[:5], list(all_annot_ids[each_class])[:5])
            # print(self.df[each_class + "_annotations_sampling_wts"].unique())
        ...
        print("max_annotation_weights: ", self.df["max_annotation_weights"].unique())
        print("final_sample_wts: ", self.df["final_sample_wts"].sum())
        self.df["final_sample_wts"] = (
    ...
    def all_sampling_weights(self):
        ...
        if self.only_seg:
            self._filter_annots_entries() ## will remove entries for seg classes for which annotations are not present. Use only for seg finetuning
        ...
        # print(self.df["final_sample_wts"].unique())
