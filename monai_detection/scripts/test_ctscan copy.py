...
if __name__ == "__main__":
    ...
    df_inc = pd.read_csv("scripts/incorrect_gt.csv")

    for i in tqdm(df_inc.index) :
        row = df_inc.loc[i]
        print(row)
        dataset = row["dataset"]
        sid = row["scan_name"]
    ...
    #     fc.Path.mkdir(
    #         fc.Path(f"/cache/fast_data_nas8/qct/shubham/det_prediction/{dataset}/"),
    #         exist_ok=True,
    #     )
        # for meter in meters:
        #     meter.reset()
        ...
        if os.path.exists(
            ...
            img["boxes"] = np.array(bx["gt_boxes"])

            # gt_boxes = boxes_tf(img)["boxes"]
            gt_boxes = bx["gt_boxes"]
            ...
            torch.save(
                out_dict,
                f"/cache/fast_data_nas8/qct/shubham/det_prediction/{dataset}/{sid}.pt",
            )
