    ...
    for i in range(imgmtx.shape[0]):
        if label is None:
            (im, m, box) = crop_and_resize(
                imgmtx[i, :, :], width=resolution[0], height=resolution[1]
            )
        else:
            (im, m, box) = crop_and_resize(
                imgmtx[i, :, :],
...
def simple_bodymask(img):
    ...
    bodymask = ndimage.binary_fill_holes(
        bodymask, structure=np.ones((3, 3))
    ).astype(int)
...
def crop_and_resize(img, mask=None, width=192, height=192):
    ...
    # img[bmask==0] = -1024 # this line
    #  removes background outside of the lung.
    # However, it has been shown problematic
    #  with narrow circular field of views that touch the lung.
    ...
    if (not mask) is None:
        ...
        mask = ndimage.zoom(
            mask, np.asarray([width, height]) / np.asarray(mask.shape), order=0
        )
...
def read_dicoms(path, primary=True, original=True):
    ...
    # dcm_parameters = []
    unique_set = []
    # need this because too often there are
    #  duplicates of dicom files with different names
    i = 0
    for fname in tqdm(allfnames):
        ...
        if filename_[0] != "DICOMDIR":
            try:
                ...
                logging.error("Unexpected error:", e, sys.exc_info()[0])
                logging.warning(
                    "Doesn't seem to be DICOM, will be skipped: ", fname
                )
    ...
    vol_unique = np.unique(
        conc, return_index=1, return_inverse=1
    )  # unique volumes
...
def postrocessing(label_image, spare=[]):
    """some post-processing mapping small label patches
     to the neighbout whith which they share the
    largest border. All connected components
     smaller than min_area will be removed
    """
    ...
    for r in tqdm(regions, desc="PostProcessing"):
        r_max_intensity = int(r.max_intensity)
        if (
            r.area < origlabels_maxsub[r_max_intensity]
            or r_max_intensity in spare
        ) and r.area > 2:
            # area>2 improves runtime because small
            #  areas 1 and 2 voxel will be ignored
            ...
            regions[regionlabels.index(mapto)].__dict__["_cache"][
                "area"
            ] += myarea
    ...
    for i in np.unique(outmask_mapped)[1:]:
        outmask[
            holefiller(keep_largest_connected_component(outmask_mapped == i))
        ] = i
...
def read_sitk_as_np(case: Union[str, sitk.Image]):
    if isinstance(case, str):
        itkimage = sitk.ReadImage(case)
    else:
        itkimage = case
